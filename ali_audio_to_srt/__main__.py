...
def 处理文件(files, 引擎):
    ...
    for index, file in enumerate(files):
        ...
        # 生成 wav
        wav路径 = f'{os.path.basename(file)}_16000hz.wav'
        ...
        # 用新线程等待识别完成
        线程 = Wait_For_Response_To_Generate_Srt(file, 识别引擎, oss文件路径, 文件url链接, 删除oss文件)
        线程列表.append(线程)
        线程.start()
...
class Wait_For_Response_To_Generate_Srt(Thread):
    def __init__(self, 文件, 识别引擎, oss文件路径, 文件url链接, 删除oss文件):
        super().__init__()
        self.文件 = 文件
        self.识别引擎 = 识别引擎
